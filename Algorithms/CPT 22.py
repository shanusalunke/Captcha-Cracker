...
def preprocess(image):
    ...
    npimg = filters.maximum_filter(npimg,3)
    npimg = dilate(npimg, 1)
    npimg = erode(npimg, 1)
    return npimg

def segment(npimg):
    return singleColumnSegmentation(npimg,7,5)
...
def recognize(images):
    ...
    chars = "" # result string
    net = loadFromPickle("C:\Python27\Captcha_Cracker\Networks\CPT 22.txt")
    ds = loadFromPickle("C:\Python27\Captcha_Cracker\Datasets\CPT 22.txt")
    for im in images:
