...
def preprocess(image):
    ...
    npimg = greyThreshold(npimg, 190)
    size = npimg.shape
    npimg = npimg[5:size[0]-10,10:size[1]-10]
    return npimg

def segment(npimg):
    return snakeSegmentation(npimg, 10)
...
def recognize(images):
    ...
    chars = "" # result string
    net = loadFromPickle("C:\Python27\Captcha_Cracker\Networks\CPT 8.txt")
    ds = loadFromPickle("C:\Python27\Captcha_Cracker\Datasets\CPT 8.txt")
    for im in images:
...
print "number", number
run(path, int(number))
#run("C:\Python27\Captcha_Cracker\Captcha Images\CPT 8", 1)
