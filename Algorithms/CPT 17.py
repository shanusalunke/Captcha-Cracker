...
def preprocess(image):
    ...
    for c in range(size[1]):
        back_count = (numpy.count_nonzero(npimg[:,c]))
        if back_count > 40:
            start_index = c
        else:
            break
    for c in reversed(range(size[1])):
        ...
        if back_count > 40:
            ...
            break
    npimg = npimg[:,start_index:end_index]
    npimg = erode(npimg, 1)
    #npimg = greyThreshold(npimg, 150)
    return npimg
...
def recognize(images):
    ...
    chars = "" # result string
    net = loadFromPickle("C:\Python27\Captcha_Cracker\Networks\CPT 17.txt")
    ds = loadFromPickle("C:\Python27\Captcha_Cracker\Datasets\CPT 17.txt")
    for im in images:
...
def run(path, number):
    ...
    for image in imageFiles:
        ...
        #saveIM(images, "CPT", "C:\Python27\Captcha_Cracker\Segmented_Characters\CPT 17\\")
...
#run("C:\Python27\Captcha_Cracker\Captcha Images\CPT 17", 61)
