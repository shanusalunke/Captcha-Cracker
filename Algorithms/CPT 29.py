...
def preprocess(image):
    ...
    npimg = filters.maximum_filter(npimg,2)
    npimg = greyThreshold(npimg, 120)
    npimg = invertBackground(npimg)
    return npimg

def segment(npimg):
    return singleColumnSegmentation(npimg,12,5)
...
def recognize(images):
    ...
    chars = "" # result string
    net = loadFromPickle("C:\Python27\Captcha_Cracker\Networks\CPT 29.txt")
    ds = loadFromPickle("C:\Python27\Captcha_Cracker\Datasets\CPT 29.txt")
    for im in images:
...
#run("C:\Python27\Captcha_Cracker\Captcha Images\CPT 29",2)
